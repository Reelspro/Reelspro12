...
const ReelsGallery = () => {
  ...
  const { reels, fetchReels, downloadReel, isLoading } = useReelStore();

  useEffect(() => {
    fetchReels();
    
    const interval = setInterval(() => {
      const currentReels = useReelStore.getState().reels || [];
      const needsPolling = currentReels.some(
        r => r && (r.status === 'queued' || r.status === 'processing')
      );
